...
> 由 `/design-demo grid` 依 `docs/research/supply-chains/grid.md` 產出。教學用、抽象。人類確認後 `/add-topic grid` 或後續逐設備 `/add-component` 建。
- type: topic
- 研究來源: docs/research/supply-chains/grid.md
...
公司不寫進 JSON。依 `docs/research/supply-chains/grid.md` §5 後續填 `companies.csv`:
