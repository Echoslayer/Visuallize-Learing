...
正典在 [`CONTEXT.md`](CONTEXT.md) / [`PLAN.md`](PLAN.md);兩條三段管線與慣例在 [`topic-playbook.md`](topic-playbook.md)、[`../machines/README.md`](../machines/README.md)。本檔只列「這次重做」的步驟與 checklist。

## 管線總覽(順序不可跳)

```
Phase A  研究供應鏈   /research-supply-chain ai-server   → docs/supply-chains/ai-server.md
Phase B  設計 demo    /design-demo ai-server             → specs/<NN>-topic-ai-server.md
...
- [ ] `/research-supply-chain ai-server` → `docs/supply-chains/ai-server.md`。
...
- [ ] `/design-demo ai-server` → `specs/<NN>-topic-ai-server.md`(沿用 design-demo 產出格式)。
...
### 每盤的 checklist
- [ ] **Research machine** — `/research-machine <盤>` → `docs/machines/<slug>.md`(公司/代號標需查證)。
...
- [x] Phase A 研究供應鏈(docs/supply-chains/ai-server.md)— 待人類校對公司/代號
- [x] Phase B 設計 demo(specs/17-topic-ai-server.md)— 待人類拍板 §7
