...
1. `/research-machine <machine>` → `docs/machines/<slug>.md`
2. `/design-machine <slug>` → `specs/<NN>-machine-<slug>.md`
3. `/add-component <slug>` → 依 spec 修改 `src/content/*.json`

規則：

- 研究寫事實：用途、外觀、輸入輸出、關鍵子系統、來源。
- 設計寫取捨：套哪個 `docs/plan/machine-patterns.md` pattern、哪些部位要 primitive 化。
