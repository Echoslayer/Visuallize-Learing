...
3. 在 `content/companies.csv` 加公司↔元件對應列(`topic,part,ticker,name`,多對多;見 ADR-0011)。
4. registry 註冊新題目。所需元件若不存在 → 先 `/add-component`。
5. `gallery/` 加該場景頁。跑 `/verify <route>`，全綠才 `/commit`。
