...
- **套用 skill `object-abstraction`**:每個元件**先調研真實物件長相**,再**組 3~8 個 primitive** 抓住可辨識的輪廓。
  **抽象 ≠ 簡陋——別用單一方塊代表一台機器**(那是目前要修的問題)。低多邊形、抓招牌特徵,不做細節。
- 形狀只用 primitive(box / cylinder / cone / tube / flow)+ `repeat`/`rotation` 組合。目標:看剪影就認得出是什麼。
- **精選必要元件**:研究可能列 N 個環節,但 demo **不必每個都建**。挑教學上**必要**的節點;次要的合併或省略,**寫明取捨理由**。寧可少而清楚。
- **決定設計細節**(這支的重點):
  - **形狀(套 object-abstraction)**:每個節點的 **primitive 組合**(調研真實長相 → 3~8 個 primitive 抓輪廓),代表什麼。不是單一形狀。
