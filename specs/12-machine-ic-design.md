...
> 由 `/design-machine ic-design` 依 `docs/machines/ic-design.md` 產出。確認後由 `/add-component` 實作。
- type: machine
- 研究來源: docs/machines/ic-design.md
...
- 不新增 `engine/kit`；`Design Workbench` 先停留在文件/spec 層。等第二個題目也需要資訊流設計節點，再更新 `docs/plan/machine-patterns.md`。
